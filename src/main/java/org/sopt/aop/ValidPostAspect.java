...
public class ValidPostAspect {
    ...
    /**
     * @param joinPoint
     * @Before("args(org.sopt.dto.request.post.PostCreateRequest)") 은 파라미터가 PostRequest인 경우 계속 적용되므로 비효율적
    ...
    public void validPostCreateRequest(JoinPoint joinPoint) {
        Object[] args = joinPoint.getArgs();
        for (Object arg : args) {
            if (arg instanceof PostCreateRequest request) {
                PostValidator.validTitle(request.title());
                PostValidator.validContent(request.content());
            }
        }
    }
        ...
        for (Object arg : args) {
            if (arg instanceof PostUpdateRequest request) {
                ...
                if (request.content().isPresent()) {
                    PostValidator.validContent(request.content().get());
                }
            }
        }
    }
